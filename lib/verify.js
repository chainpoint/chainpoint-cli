...
async function executeAsync(yargs, argv) {
  ...
  if (!verifyAll) {
    ...
    let verifyResults
    try {
      if (spv) verifyResults = await SPVVerifyProofs(proofData)
      else verifyResults = await VerifyProofs(proofData)
    } catch (error) {
...
async function SPVVerifyProofs(proofData) {
  let verifyResponse
  // create a config object to read bcoin client options
  const config = new Config('chainpoint')
  config.load({
    file: true,
    argv: true,
    env: true
  ...
  const network = Network.get(config.str('network', Network.primary))
  ...
  try {
    proofData.forEach(async ({ proof }) => {
      const response = await chp.evaluateProofs([proof])
      const btc = response.filter(r => r.type === 'btc' && r.anchor_id)
      const validated = btc.map(async anchor => {
        const hash = await client.execute('getblockhash', [
          parseInt(anchor.anchor_id, 10)
        ])
        const header = await client.execute('getblockheader', [hash])
        console.log('match? ', header.merkleroot === anchor.expected_value)
      })
    })
  } catch (error) {
    console.error(`Could not verify proofs : ${error}`)
  }
}
