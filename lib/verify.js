...
async function executeAsync (yargs, argv) {
  ...
  if (!verifyAll) {
    ...
    try {
      ...
      let verifyResults
      try {
        verifyResults = await ProcessVerifyTasks(verifyTasks, baseURI)
      } catch (error) {
...
function ProcessVerifyTasks (verifyTasks, baseURI, callback) {
  callback = callback || function () { }

  // Validate callback is a function
  if (!utils.isFunction(callback)) throw new Error('callback arg must be a function')
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      async.mapLimit(uniqueAnchorList, 20, (uniqueAnchor, mapCallback) => {
        confirmExpectedValue(uniqueAnchor, baseURI, mapCallback)
      }, (err, confirmResults) => {
        if (err) throw new Error('could not confirm expected values')
...
function confirmExpectedValue (anchorInfo, baseURI, callback) {
  callback = callback || function () { }
        ...
        case 'cal':
          ...
          request(options, (error, response, body) => {
            if (error) {
              ...
              let blockHash = body
              returnResult = (blockHash === expectedValue) ? 'valid' : 'invalid'
            }
        ...
        case 'btc':
          ...
          request(options, (error, response, body) => {
            if (error) {
              ...
              returnResult = (blockDataVal === expectedValue) ? 'valid' : 'invalid'
            }
            resolve(returnResult)
            return callback(null, returnResult)
          })
