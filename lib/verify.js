...
function execute (yargs, argv) {
  ...
  if (!updateAll) {
    ...
    if (!isValidHashId) {
      yargs.showHelp()
      console.log(`Verify error: Missing or invalid hash_id_node`)
      return
    }
    ...
    async.waterfall([
      ...
      (stackConfig, hashDb, wfCallback) => {
        updateCmd.updateHashesByHashIdNode(stackConfig, hashDb, [hashIdNode], baseURI, (err, updateResults) => {
          if (err) return wfCallback(err)
          if (updateResults[0].proof === null) return wfCallback({ message: `Cannot find proof for hash with hash_id_node = ${hashIdNode}` })
          return wfCallback(null, stackConfig, hashDb)
        })
      },
      (stackConfig, hashDb, wfCallback) => {
      ...
      (stackConfig, hashDb, hashItems, wfCallback) => {
        ...
        hashItems = hashItems.filter((hashItem) => {
          if (hashItem.proof == null) {
            console.log(`Proof for hash with hash_id_node = ${hashItem._id} is not yet available`)
            return false
          }
          return true
        })
...
function outputVerifyResults (verifyResults) {
  _.each(verifyResults, (proof) => {
    ...
    console.log(`${proof.hash_id_node} | ${proof.status} | ${proofAnchorsString}`)
  })
}
