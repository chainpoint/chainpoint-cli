  ...
  if (!verifyAll) {
    ...
    try {
      ...
      for (let x = 0; x < hashIdNodeSegments.length; x++) {
        await updateCmd.updateHashesByHashIdNodeAsync(nodeConfig, hashDb, hashIdNodeSegments[x], baseURI)
      }
    } catch (error) {
      output.addErrorResult({
        message: error.message
    ...
    for (let x = 0; x < proofSegments.length; x++) {
      // verify proofs
      let verifyResults = await verifyProofsAsync(nodeConfig, proofSegments[x], baseURI)
