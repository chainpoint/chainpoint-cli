...
async function executeAsync(yargs, argv) {
  ...
  if (!verifyAll) {
    ...
    let verifyResults
    try {
      verifyResults = await VerifyProofs(proofData, spv)
    } catch (error) {
...
async function VerifyProofs(proofData, spv) {
  let verifyResponse
  ...
  try {
    if (spv) {
      ...
      for (var nodeUri in proofDataURIGroups) {
        if (spv) {
          let task = verifySetAsync(proofDataURIGroups[nodeUri])
          verifyTasks.push(task)
        } else if (proofDataURIGroups.hasOwnProperty(nodeUri)) {
