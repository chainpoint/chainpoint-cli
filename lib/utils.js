...
async function getStackConfigAsync (baseURI) {
  ...
  try {
    ...
    throw new Error(`Error retrieving Node config: ${error.message}`)
  }
}

function log (contentArray, isError, isQuiet, isJson) {
  ...
  if (!isError) {
    ...
    if (isJson) {
      console.error({
        success: false,
        message: contentArray[0]
      })
    } else {
      console.error(contentArray[0])
    }
  }
}
