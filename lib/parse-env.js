...
const CHAINPOINT_NODE_API_BASE_URI_DEFAULT = 'http://127.0.12.1:9090'
...
let envDefinitions = {
  ...
  CHAINPOINT_NODE_API_BASE_URI: envalid.url({ default: CHAINPOINT_NODE_API_BASE_URI_DEFAULT, desc: 'Base URI for the Chainpoint services to consume' })
...
function initConfig () {
  ...
  if (!fs.existsSync(untildify(pathToConfigFile))) {
    ...
    let content = `CHAINPOINT_NODE_API_BASE_URI=${CHAINPOINT_NODE_API_BASE_URI_DEFAULT}`
