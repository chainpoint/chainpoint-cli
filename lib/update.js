...
async function executeAsync(yargs, argv) {
  ...
  if (!updateAll) {
    ...
    try {
      ...
      let updateResults = await updateHashesByHashIdNodeAsync(
        hashDb,
        proofHandles
      )
    ...
    for (let x = 0; x < proofHandleSegments.length; x++) {
      updateTasks.push(async () => {
        return updateHashesByHashIdNodeAsync(hashDb, proofHandleSegments[x])
      })
    }
  ...
  try {
    ...
    for (let x = 0; x < proofHandles.length; x++) {
      updateResults.push({
        ...
        message: 'Unable to update at this time, try again'
      })
    }
...
async function buildProofHandleSegments(hashDb) {
  ...
  hashItems = await hashDb.findAsync(
