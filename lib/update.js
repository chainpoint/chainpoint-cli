...
async function executeAsync (yargs, argv) {
  ...
  if (!updateAll) {
    ...
    for (let x = 0; x < hashIdNodeSegments.length; x++) {
      let updateResults = await updateHashesByHashIdNodeAsync(nodeConfigs[hashIdNodeSegments[x].nodeUri], hashDb, hashIdNodeSegments[x].hashIdNodes, hashIdNodeSegments[x].nodeUri)
