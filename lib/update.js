...
function execute (yargs, argv) {
  ...
  if (!updateAll) {
    ...
    if (!isValidHashId) {
      yargs.showHelp()
      console.error(`Update error: Missing or invalid hash_id_node`)
      return
    }
    ...
    async.waterfall([
      ...
      (stackConfig, hashDb, wfCallback) => {
        updateHashesByHashIdNode(stackConfig, hashDb, [hashIdNode], baseURI, (err, updateResults) => {
          if (err) return wfCallback(err)
          outputUpdateResults(updateResults, quiet)
    ...
    async.waterfall([
      ...
      (stackConfig, hashDb, hashItems, wfCallback) => {
        ...
        async.eachSeries(hashIdNodeSegments, (hashIdNodeSegment, eachCallback) => {
          ...
          if (err) return wfCallback(err)
          outputUpdateResults(allResults, quiet)
          return wfCallback(null)
        })
      }
    ], (err) => {
...
function outputUpdateResults (updateResults, quiet) {
  _.each(updateResults, (hashItem) => {
    switch (hashItem.proof) {
      case null:
        if (!quiet) console.log(`Hash with hash_id_node = ${hashItem.hash_id_node} has no proof data`)
        break
      case '':
        if (!quiet) console.log(`Could not parse proof for hash with hash_id_node  = ${hashItem.hash_id_node}`)
        break
      default:
        if (!quiet) console.log(`Hash with hash_id_node = ${hashItem.hash_id_node} updated with latest proof data`)
    }
  })
}
