...
function execute (yargs, argv) {
  ...
  if (hasArgument) {
    ...
    async.series([
      ...
      (callback) => {
        updateHashesByHashId([hashId], baseURI, (err, updateResults) => {
    ...
    async.waterfall([
      ...
      (hashItems, callback) => {
        ...
        async.eachSeries(hashIdSegments, (hashIdSegment, eachCallback) => {
          ...
          updateHashesByHashId(hashIdSegment, baseURI, (err, updateResults) => {
...
function updateHashesByHashId (hashIdArray, baseURI, callback) {
        ...
        if (hashItem.proof === null) {
          ...
          HashItem.update(updateData, { where: { hashId: hashItem.hash_id } })
            .then((result) => {
              ...
              return eachCallback(`HashItem update error: ${err.message}`)
            })
        }
      }, (err) => {
        if (err) return wfCallback(err)
