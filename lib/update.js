...
function execute (yargs, argv) {
  ...
  if (!updateAll) {
    ...
    if (!isValidHashId) {
      yargs.showHelp()
      console.log(`Update error: Missing or invalid hash_id_node`)
      return
    }
...
function outputUpdateResults (updateResults) {
  _.each(updateResults, (hashItem) => {
    switch (hashItem.proof) {
      case null:
        console.log(`Hash with hash_id_node = ${hashItem.hash_id_node} has no proof data`)
        break
      case '':
        console.log(`Could not parse proof for hash with hash_id_node  = ${hashItem.hash_id_node}`)
        break
      default:
        console.log(`Hash with hash_id_node = ${hashItem.hash_id_node} updated with latest proof data`)
    }
  })
}
