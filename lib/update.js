  ...
  for (let x = 0; x < responseBody.length; x++) {
    ...
    if (hashItem.proof === null) {
      ...
      try {
        ...
        updateResults.push({ hash_id_node: hashItem.hash_id_node, proof: hashItem.proof, anchors: stateString })
      } catch (error) {
  ...
  _.each(updateResults, (hashItem) => {
    switch (hashItem.proof) {
      ...
      case '':
        ...
        break
      default:
        items.push(`Hash with hash_id_node = ${hashItem.hash_id_node} updated : ${hashItem.anchors}`)
    }
  })
