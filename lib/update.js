...
function execute (yargs, argv) {
  ...
  if (hasArgument) {
    ...
    if (utils.hashIdExpired(hashId)) { // this hash has expired
      console.log(`Update error: Expired hash_id: ${hashId}`)
      return
    }
    ...
    async.waterfall([
      ...
      (hashDb, wfCallback) => {
        updateHashesByHashId(hashDb, [hashId], baseURI, (err, updateResults) => {
    ...
    async.waterfall([
      ...
      (hashDb, wfCallback) => {
        ...
        hashDb.find({ createdAt: { $gte: oldestDate } }, { _id: 1 }, (err, hashItems) => {
          if (err) return wfCallback(err)
          return wfCallback(null, hashDb, hashItems)
        })
      },
      (hashDb, hashItems, wfCallback) => {
        // get hashId array
        ...
        async.eachSeries(hashIdSegments, (hashIdSegment, eachCallback) => {
          ...
          updateHashesByHashId(hashDb, hashIdSegment, baseURI, (err, updateResults) => {
...
function updateHashesByHashId (hashDb, hashIdArray, baseURI, callback) {
