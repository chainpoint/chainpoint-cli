...
async function executeAsync (yargs, argv) {
  ...
  if (!updateAll) {
    ...
    try {
      ...
      let stackConfig = await utils.getStackConfigAsync(targetNodeUri)
    ...
    try {
      ...
      // create a dictionary of stack configs for all unique nodeUris
      for (let x = 0; x < nodeUris.length; x++) {
        try {
          let stackConfig = await utils.getStackConfigAsync(nodeUris[x])
          stackConfigs[nodeUris[x]] = stackConfig
        } catch (error) {
      ...
      hashItems = hashItems.filter((hashItem) => {
        return !utils.hashIdExpired(stackConfigs[hashItem.nodeUri].proof_expire_minutes, hashItem._id)
      })
    } catch (error) {
      output.addErrorResult({
        message: error.message
    ...
    for (let x = 0; x < nodeUriGroupsArray.length; x++) {
      ...
      while (nodeUriGroupsArray[x].hashIdNodes.length > 0) {
        hashIdNodeSegments.push({
          ...
          hashIdNodes: nodeUriGroupsArray[x].hashIdNodes.splice(0, stackConfigs[nodeUriGroupsArray[x].nodeUri].get_proofs_max_rest)
        })
      }
    }
    ...
    for (let x = 0; x < hashIdNodeSegments.length; x++) {
      let updateResults = await updateHashesByHashIdNodeAsync(stackConfigs[hashIdNodeSegments[x].nodeUri], hashDb, hashIdNodeSegments[x].hashIdNodes, hashIdNodeSegments[x].nodeUri)
