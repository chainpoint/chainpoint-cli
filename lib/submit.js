...
function execute (yargs, argv) {
  ...
  if (argCount < 2) { // there is no hash parameter
    yargs.showHelp()
    console.error('Submit error: Missing hash argument')
  ...
  hashes = hashes.filter((hash) => {
    if (utils.hashIsValid(hash)) {
      ...
      console.error(`Submit error: Invalid hash : ${hash}`)
      return false
    }
  })
  ...
  async.waterfall([
    ...
    (stackConfig, hashDb, wfCallback) => {
      ...
      async.eachSeries(hashSegments, (hashSegment, eachCallback) => {
        ...
        if (err) return wfCallback(err)
        outputSubmitResults(allResults, quiet)
...
function outputSubmitResults (submitResults, quiet) {
  _.each(submitResults, (hashItem) => {
    if (!quiet) console.log(`hash_id_node : ${hashItem._id}`)
  })
}
