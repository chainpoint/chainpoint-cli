...
async function submitHashesAsync (stackConfig, hashDb, hashArray, baseURI) {
  ...
  for (let x = 0; x < responseBody.hashes.length; x++) {
    ...
    try {
      // validate BLAKE2s
      let hashTimestampMS = uuidTime.v1(hashItem.hash_id_node)
      ...
      if (embeddedData === expectedData) {
        ...
        await hashDb.insertAsync(row)
        row.state = 'submitted'
      } else {
        // the BLAKE2 hash is invalid
        row.state = 'refused - cannot validate BLAKE2s embedded data'
      }
...
function outputSubmitResults (submitResults, quiet, json) {
  ...
  _.each(submitResults, (hashItem) => {
    items.push(`hash : ${hashItem.hash} : hash_id_node : ${hashItem._id} : ${hashItem.state}`)
  })
