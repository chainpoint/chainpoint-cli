...
async function executeAsync(yargs, argv) {
  ...
  let baseURI = argv.gatewayUri === 'http://0.0.0.0' ? null : argv.gatewayUri

  let argCount = argv._.length

  let hashes = []
  if (argCount < 2) {
    // there is no hash parameter
    output.addErrorResult({
      message: `missing hash argument`
    })
    output.display(quiet, json)
  } else if (argCount === 2) {
    // this is one hash, or a CSV of hashes
    hashes = argv._[1].split(',')
  } else {
    // this is a list of multiple hashes
  ...
  for (let x = 0; x < hashSegments.length; x++) {
    submitTasks.push(() => submitHashesAsync(hashDb, hashSegments[x], baseURI))
  }
  ...
  for (let x = 0; x < allResults.length; x++) {
    if (allResults[x].success) {
      output.addSuccessResult({
        proof_id: allResults[x].proof_id,
...
async function submitHashesAsync(hashDb, hashArray, baseURI) {
  ...
  for (let x = 0; x < submitResponse.length; x++) {
    ...
    try {
      // validate BLAKE2s
      let hashTimestampMS = parseInt(uuidTime.v1(hashItem.proofId))
      ...
      let embeddedData = hashItem.proofId.slice(24)
      if (embeddedData === expectedData) {
        // the BLAKE2 hash has been validated
        blakeValid = true
        await hashDb.insertAsync({
          _id: hashItem.proofId,
          hash: hashItem.hash,
          gatewayUri: hashItem.uri
        })
      }
      let submitResult = {}
      submitResult.success = blakeValid
      submitResult.proof_id = hashItem.proofId
