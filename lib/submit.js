...
async function executeAsync (yargs, argv) {
  ...
  let hashDb
  try {
    let nodeConfigFound = false
    for (let x = 0; x < baseURIs.length; x++) {
      try {
        let nodeConfig = await utils.getNodeConfigAsync(baseURIs[x])
        nodeConfigs.push(nodeConfig)
        nodeConfigFound = true
      } catch (error) {
        nodeConfigs.push(null)
      }
    }
    if (!nodeConfigFound) throw new Error('Could not retrieve Node configurations')
  ...
  // create submiot tasks for each Node to submit to in baseURIs
  let allResults = []
  let submitTasks = []
  for (let x = 0; x < baseURIs.length; x++) {
    // if the Node configuration was not received, dont bother submitting hashes to this Node
    if (!nodeConfigs[x]) continue
    let hashSegments = []
    ...
    while (workingHashSet.length > 0) hashSegments.push(workingHashSet.splice(0, nodeConfigs[x].post_hashes_max))

    for (let y = 0; y < hashSegments.length; y++) {
      submitTasks.push(async () => { return submitHashesAsync(nodeConfigs[x], hashDb, hashSegments[y], baseURIs[x]) })
    }
  }
