...
function execute (yargs, argv) {
  ...
  if (!isBinaryFile && !isJSONFile) {
    console.error('Import error: Unknown file type')
    return
  }

  let proofFileData = utils.readFile(filePath, isBinaryFile)
  if (!proofFileData) {
    console.error('Import error: Cannot find file at that path')
    return
  }
  ...
  if (!proofObject) {
    console.error('Import error: Not a valid Chainpoint v3 proof')
    return
  }
  ...
  async.waterfall([
    ...
    (hashDb, wfcallback) => {
      ...
      hashDb.update({ _id: proofObject.hash_id_node }, row, { upsert: true }, (err, numReplaced, upsert) => {
        if (err) return wfcallback(err)
        if (!quiet) console.log(`${filePath} imported`)
