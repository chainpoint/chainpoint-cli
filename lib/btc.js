...
function displayOutputResults(resultsArray, outputObj, quiet, json) {
  for (let x = 0; x < resultsArray.length; x++) {
    let tx
    if (resultsArray[x].raw_btc_tx) {
      tx = TX.fromRaw(resultsArray[x].raw_btc_tx, 'hex')
      outputObj.addSuccessResult({
        ...
        // but showing the count to provide additional information
        prevout_count: tx.inputs.length,
        prevout_1: tx.inputs[0].prevout.rhash()
      })
    }
  }
...
async function EvaluateProofs(proofs) {
  ...
  try {
    evaluateResponse = await chp.getProofTxs(proofs)
  } catch (error) {
